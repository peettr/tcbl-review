...
热带气旋（Tropical Cyclone, 台风，包括飓风/台风/旋风）可产生极端的近地面风速，对建成环境构成严重威胁。这些极端风速危及各类土木工程结构，包括低层和高层建筑、大跨度桥梁、风力发电机组以及输电塔线系统。加之台风引发的次生灾害（如风暴潮、洪水和大面积停电），台风往往造成毁灭性的社会经济影响。例如，2005年5级飓风Katrina最大持续风速约280 km/h，袭击美国后造成约1250亿美元经济损失和1392人死亡（[Wikipedia 2025](#ref-wikipedia-2025)）。近年来，2024年大西洋飓风季造成了2322.7亿美元经济损失，成为有记录以来损失第二高的飓风季（[Wikipedia 2025](#ref-wikipedia-2025)）。
...
其中，性能指标（PI，如损失）的互补累积分布$G(PI)$由一系列条件概率分布函数定义；DM、EDP、IP和SP分别表示损伤参数、工程需求参数（如风致结构响应）、风-结构相互作用参数（如风压系数）和结构属性参数；$\lambda(IM)$为灾害强度测度的概率分布函数，通常用风速和风向的联合分布表示。在基于模拟的PBWE中，式(@eq-1)通常通过蒙特卡罗方法求解：基于台风特征参数的统计特性生成台风风场样本集（即台风模拟），结合DM、EDP、IP和SP的抽样实现，得到相应的PI样本集。对这些样本进行统计分析，即可获得PBWE所需的概率特性估计。此外，概率分布$\lambda(IM)$可直接由模拟台风样本的统计特性估算。
